...
# LCG
# X(i+1) = (aX(i) + c) % m
# c = 0	=> multiplicative LCM
# c ≠ 0 => mixed LCM

maximal period:
1. m = 2 ** b, c ≠ 0, a = 1 + 4k, gcd(c, m) = 1		=> p = m = 2 ** b
2. m = 2 ** b, c = 0, a = {3, 5} + 8k, X0 is odd	=> p = m/4 = 2 ** (b-2)
3. m = prime, c = 0, k - smallest integer such that 
	a**k - 1 is divisible by m is k = m - 1			=> p = m - 1
'''
